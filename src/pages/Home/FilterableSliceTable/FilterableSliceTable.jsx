...
export const FilterableSliceTableInternal = ({ fetchInjected = fetch }) => {
  const [selectedYear, setSelectedYear] = useState("2020");
  const [selectedTranches, setSelectedTranches] = useState([]);

  useEffect(() => {
    fetchInjected(`/Slices?year=${selectedYear}`)
      .then((response) => response.json())
      .then((json) => {
        setSelectedTranches(json);
      });
  }, [selectedYear, fetchInjected]);
...
export const FilterableSliceTable = () => <FilterableSliceTableInternal />;
